...
function videoStatUpdate() {
	if ($('#kazoku-media').attr('src') !== '') {
		...
		if (typeof tX == "number") {
			socket.emit('player status', { playState: $('#kazoku-media')[0].paused, position: mediaCurrent, X: x , Y: y, tX: tX, tY: tY });
		}
		else {
...
socket.on('recv room stat', function(data) {
	for (var user in data){
		if (data.hasOwnProperty(user)) {
			var stat = data[user];
			if ($('#kz-floor div[data-user=' + user + ']').length === 0) {
				$('#kz-floor').append(
					$('<div />').attr('data-user', user)
								.addClass('kz-user')
								.append(
									$('<span />').addClass('kz-name') 
												.text(user)
								)
								.append(
									$('<div />').addClass('kz-head')
												.css('background-image', 'url(/account.ps?pic=' + user + ')')
								)
								.append(
									$('<span />').addClass('ping kz-ping')
												.text('--')
								)
								.append(
									$('<span />').addClass('status kz-status')
												.text('--')
								)
				);
				if (user == name) {
					me = $('#kz-floor div[data-user=' + name + ']');
				}
			...
			$('#kz-floor div[data-user=' + user + '] span.ping').text(stat.ping);
		}
	}
});
...
socket.on('player status', function(data) {
	if (Math.abs(data.offset) > 1) {
		$('#kz-floor div[data-user=' + data.user + '] span.status').text(data.offset);
	}
	else {
		$('#kz-floor div[data-user=' + data.user + '] span.status').html("<i class='fa fa-check'></i>");
	}
	// rudimentary.
	if (typeof data.Y == "number" && data.user !== name) {
		$('#kz-floor div[data-user=' + data.user + ']').css({ left: data.X + "px", top: data.Y + "px" });
		$('#kz-floor div[data-user=' + data.user + ']').css("background-position", data.tY + "px " + data.tX + "px");
	}
});
/* > chat */
socket.on('recv chat message', function(data) {
	kazokuLog(data.username, data.message);
	$('#kz-floor div[data-user=' + data.username + ']').protipShow({ title: data.message, trigger: "sticky", position: "top" });
	setTimeout("$(\"#kz-floor div[data-user='" + data.username + "']\").protipHide()", 2000);
});
